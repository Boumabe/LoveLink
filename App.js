...
function Accueil({ langue, onJeu, onChangerLangue }) {
  ...
  const jeux = [
    { id:'quiz', emoji:'💞', nom:tg.quiz, desc:tg.quizDesc, couleur:'#FF3D6B' },
    { id:'kay',  emoji:'🪨', nom:tg.kay,  desc:tg.kayDesc,  couleur:'#F5A623' },
    { id:'devine',emoji:'🔍',nom:tg.devine,desc:tg.devineDesc,couleur:'#10D9A0' },
  ];
    ...
    <ScrollView style={s.ecran} contentContainerStyle={[s.ecranC,{alignItems:'center',paddingTop:40}]}>
      ...
      {iaOk !== null && (
        <View style={[s.iaBadge,{backgroundColor:iaOk?'rgba(16,217,160,0.1)':'rgba(245,166,35,0.1)',borderColor:iaOk?'rgba(16,217,160,0.4)':'rgba(245,166,35,0.4)',marginBottom:24}]}>
          <Text style={{color:iaOk?'#10D9A0':'#F5A623',fontSize:11}}>
            {iaOk ? (TQ[langue]||TQ.fr).iaActif : (TQ[langue]||TQ.fr).iaNonDispo}
          </Text>
        </View>
      )}
      ...
      {jeux.map(j => (
        <TouchableOpacity key={j.id} onPress={() => onJeu(j.id)} style={[s.jeuCard,{borderColor:`${j.couleur}40`}]}>
          ...
          <Text style={{color:`${j.couleur}80`,fontSize:18}}>›</Text>
        </TouchableOpacity>
      ))}
    </ScrollView>
  );
...
function ChoixNiveau({ t, langue, sessionCode, estJ1, onDemarrer }) {
  ...
  useEffect(()=>{
    if(!estJ1){
      unsubRef.current=fbListen(`sessions/${sessionCode}/config`,val=>{
        if(val?.niveau&&val?.categorie&&val?.confirme){onDemarrer(val.niveau,val.categorie);if(unsubRef.current)unsubRef.current();}
      });
  ...
  if(!estJ1) return(
    <View style={s.centre}><Text style={s.bigE}>⏳</Text><Text style={s.titre2}>{t.partnerChoosing}</Text><ActivityIndicator color="#FF3D6B" size="large" style={{marginTop:20}}/></View>
  );
        ...
        {cats.map(c=>{
          ...
          return(
            <TouchableOpacity key={c} style={[s.catPill,actif&&s.catPillActif]} onPress={()=>setCategorie(c)}>
              <Text style={{fontSize:20}}>{parts[0]}</Text>
              <Text style={[s.catNom,actif&&{color:'#F9F2E7'}]}>{parts.slice(1).join(' ')}</Text>
            </TouchableOpacity>
          );
        })}
      </View>
...
function Quiz({ t, langue, sessionCode, estJ1, niveauId, categorieKey, onFin }) {
  ...
  async function charger(){
    if(estJ1){
      const hist=await getHistorique(sessionCode);
      const question=await genererQuestion(langue,categorieKey,niveauId,hist);
      ...
      <ScrollView style={s.ecran} contentContainerStyle={s.ecranC}>
        ...
        {!modePerso&&(
          <View style={{gap:9,marginBottom:10}}>
            ...
            {choisi===null&&(
              <TouchableOpacity style={[s.repN,{borderColor:'rgba(245,166,35,0.4)',backgroundColor:'rgba(245,166,35,0.06)'}]} onPress={()=>setModePerso(true)}>
                <View style={[s.lettre,{borderColor:'rgba(245,166,35,0.4)'}]}><Text style={{color:'#F5A623',fontSize:14}}>✏️</Text></View>
        ...
        {modePerso&&choisi===null&&(
          <View style={s.persoBox}>
            ...
            <View style={{flexDirection:'row',gap:10,marginTop:10}}>
              <TouchableOpacity style={[s.btnV,{flex:1,marginTop:0}]} onPress={()=>setModePerso(false)}><Text style={s.btnVT}>← Retour</Text></TouchableOpacity>
              <TouchableOpacity style={[s.btnR,{flex:1,marginBottom:0}]} onPress={validerPerso}><Text style={s.btnRT}>{t.valider}</Text></TouchableOpacity>
            </View>
          </View>
        )}
        {!lesDeux&&<View style={s.msgBox}><Text style={s.msgTxt}>{choisi===null?t.choixReponse:t.attentePartner}</Text></View>}
        {lesDeux&&(
          <View style={[s.msgBox,{borderColor:memeReponse?'rgba(16,217,160,0.4)':'rgba(245,166,35,0.4)',backgroundColor:memeReponse?'rgba(16,217,160,0.08)':'rgba(245,166,35,0.08)'}]}>
            <Text style={[s.msgTxt,{color:memeReponse?'#10D9A0':'#F5A623',fontWeight:'700',fontSize:15}]}>{memeReponse?t.memeReponse:t.parlezEnsemble}</Text>
            <Text style={[s.msgTxt,{marginTop:6,fontSize:12}]}>{memeReponse?t.complicite:t.differences}</Text>
          </View>
        )}
  ...
  return(
    <ScrollView style={s.ecran} contentContainerStyle={[s.ecranC,{alignItems:'center'}]}>
      ...
      <View style={s.messageFinal}>
        ...
        <Text style={{color:'#F9F2E7',fontSize:15,textAlign:'center',lineHeight:24,fontWeight:'500'}}>Chaque question vous rapproche,{'\n'}même à des milliers de kilomètres. 💕</Text>
      </View>
...
function KayPit({stones,isActive,isSelected,isLanding,isCaptured,isPaused,isNextManuel,onPress,disabled,player,color,p1Color,p2Color}){
  ...
  if(isCaptured){bg=`${color.main}35`;bc=color.main;transform=[{scale:1.08}];}
  else if(isNextManuel){bg='rgba(255,255,255,0.18)';bc='rgba(255,255,255,0.7)';transform=[{scale:1.1}];}
  else if(isSelected){bg='rgba(255,255,255,0.14)';bc=border;transform=[{scale:1.1}];}
  else if(isLanding){bg=`${color.main}20`;bc=`${color.main}cc`;transform=[{scale:1.06}];}
  else if(isPaused){bg='rgba(255,255,255,0.12)';bc='rgba(255,255,255,0.5)';transform=[{scale:1.05}];}
  else if(isActive){bg='rgba(255,255,255,0.07)';bc='rgba(255,255,255,0.2)';}
  return(
    <TouchableOpacity onPress={disabled?null:onPress} activeOpacity={disabled?1:0.7} style={{width:54,height:54,borderRadius:27,backgroundColor:bg,borderWidth:1.5,borderColor:bc,transform,alignItems:'center',justifyContent:'center',flexWrap:'wrap',padding:6,overflow:'hidden'}}>
      <View style={{flexDirection:'row',flexWrap:'wrap',gap:2,justifyContent:'center',alignItems:'center'}}>
        {Array(Math.min(stones,9)).fill(0).map((_,i)=>(
          <View key={i} style={{width:stones>6?8:11,height:stones>6?8:11,borderRadius:6,backgroundColor:color.main,opacity:0.9}}/>
        ))}
      </View>
      <Text style={{position:'absolute',bottom:3,fontSize:10,fontWeight:'900',color:'rgba(249,242,231,0.6)',backgroundColor:'rgba(0,0,0,0.5)',borderRadius:6,paddingHorizontal:4,lineHeight:14}}>{stones}</Text>
    </TouchableOpacity>
  );
}
...
function KayGame({ langue, onRetour }) {
  ...
  useEffect(()=>{
    ...
    setCurrentPlayer(first);setShowNotif(true);setPhase('notif');
  },[]);

  useEffect(()=>{
    if(phase==='notif'){const timer=setTimeout(()=>{setShowNotif(false);setPhase('selectCase');},1800);return()=>clearTimeout(timer);}
  },[phase,currentPlayer]);
  ...
  async function handlePitPress(player,index){
    ...
    animating.current=true;setPhase('playing');
    await kaySleep(300);
    const b={p1:[...board.p1],p2:[...board.p2]};
    const rs={...roundScores};
    if(isManuel)await kayManuel(player,index,b,rs);
    else await kayDistribute(player,index,b,rs);
  }

  async function kayDistribute(player,startIndex,b,rs){
    const p1Len=b.p1.length,p2Len=b.p2.length;
    let stones=b[`p${player}`][startIndex];
    b[`p${player}`][startIndex]=0;setBoard({p1:[...b.p1],p2:[...b.p2]});setSelectedCase(null);
    await kaySleep(200);
    ...
    while(stones>0){
      ...
      b[`p${tP}`][tI]++;stones--;
      setLandingPit({player:tP,index:tI});setBoard({p1:[...b.p1],p2:[...b.p2]});
      ...
      if(b[`p${tP}`][tI]===4){
        ...
        setRoundScores({...rs});setBoard({p1:[...b.p1],p2:[...b.p2]});
        await kaySleep(KAY_PAUSE_CAP);setCapturedPit(null);setShowCapture(false);
        if(stones===0){kayFinish(player,b,rs);return;}
        continue;
      }
      if(stones===0){
        if(wasEmpty){kayFinish(player,b,rs);return;}
        const sh=b[`p${tP}`][tI];
        if(sh>0){
          setPausedPit({player:tP,index:tI});await kaySleep(KAY_PAUSE_C);setPausedPit(null);
          stones=sh;b[`p${tP}`][tI]=0;setBoard({p1:[...b.p1],p2:[...b.p2]});await kaySleep(200);
        }
      }
    }
    kayFinish(player,b,rs);
  }

  async function kayManuel(player,startIndex,b,rs){
    const p1Len=b.p1.length,p2Len=b.p2.length;
    let stones=b[`p${player}`][startIndex];
    b[`p${player}`][startIndex]=0;setBoard({p1:[...b.p1],p2:[...b.p2]});setSelectedCase(null);
    await kaySleep(200);
    let gPos=player===1?startIndex:p1Len+startIndex;
    await kayManuelLoop(player,gPos,stones,b,rs,p1Len,p2Len);
  }

  async function kayManuelLoop(player,gPos,stones,b,rs,p1Len,p2Len){
    if(stones===0){kayFinish(player,b,rs);return;}
    ...
    setBoard({p1:[...b.p1],p2:[...b.p2]});setManuelStones(stones);
    ...
    if(b[`p${nP}`][nI]===4){
      ...
      await kaySleep(KAY_PAUSE_CAP);setCapturedPit(null);setShowCapture(false);
      if(stones===0){kayFinish(player,b,rs);return;}
      await kayManuelLoop(player,nextG,stones,b,rs,p1Len,p2Len);return;
    }
    if(stones===0){
      if(wasEmpty){kayFinish(player,b,rs);return;}
      const sh=b[`p${nP}`][nI];
      if(sh>0){
        ...
        stones=sh;b[`p${nP}`][nI]=0;setBoard({p1:[...b.p1],p2:[...b.p2]});await kaySleep(200);
      }
    }
    await kayManuelLoop(player,nextG,stones,b,rs,p1Len,p2Len);
  }

  function kayFinish(player,b,rs){
    ...
    if(total<=KAY_END){
      ...
      setRoundWinner(winner);setScores(sc=>({p1:sc.p1+(winner===1?1:0),p2:sc.p2+(winner===2?1:0)}));
      setPhase('roundOver');return;
    }
    const next=player===1?2:1;setCurrentPlayer(next);setShowNotif(true);setPhase('notif');
  }
  ...
  function startNextRound(winner){
    ...
    if(n1===0||n2===0){setPhase('gameOver');return;}
    setP1Cases(n1);setP2Cases(n2);setBoard(kayInitBoard(n1,n2));setRoundScores({p1:0,p2:0});
    setRoundNum(r=>r+1);setRoundWinner(null);setSelectedCase(null);setLandingPit(null);
    setCapturedPit(null);setPausedPit(null);setNextValidPit(null);setManuelStones(0);
    animating.current=false;
    const first=Math.random()<0.5?1:2;setCurrentPlayer(first);setShowNotif(true);setPhase('notif');
  }

  function kayRestart(){
    setP1Cases(KAY_MAX);setP2Cases(KAY_MAX);setBoard(kayInitBoard(KAY_MAX,KAY_MAX));
    setScores({p1:0,p2:0});setRoundScores({p1:0,p2:0});setRoundNum(1);setRoundWinner(null);
    setSelectedCase(null);setLandingPit(null);setCapturedPit(null);setPausedPit(null);
    setNextValidPit(null);setManuelStones(0);setShowCapture(false);animating.current=false;
    manuelResolveRef.current=null;
    const first=Math.random()<0.5?1:2;setCurrentPlayer(first);setShowNotif(true);setPhase('notif');
  }
  ...
  return(
    <ScrollView style={s.ecran} contentContainerStyle={[s.ecranC,{alignItems:'center',paddingBottom:60}]}>
      <TouchableOpacity onPress={onRetour} style={{alignSelf:'flex-start',marginBottom:8}}>
        <Text style={s.retour}>← {(TG[langue]||TG.fr).retour.replace('← ','')}</Text>
      </TouchableOpacity>
      ...
      {/* Flash capture */}
      {showCapture&&(
        <View style={{position:'absolute',top:'38%',left:'50%',transform:[{translateX:-60}],zIndex:50,backgroundColor:`${color.main}22`,borderWidth:2,borderColor:color.main,borderRadius:50,paddingVertical:10,paddingHorizontal:24}}>
      ...
      <Text style={[s.sous,{marginBottom:16}]}>{t.subtitle.toUpperCase()}</Text>
      ...
      <View style={{flexDirection:'row',gap:8,marginBottom:14,alignItems:'center'}}>
        ...
        <View style={{alignItems:'center'}}>
          ...
          <Text style={{fontSize:14,fontWeight:'700',color:'rgba(249,242,231,0.5)',marginTop:4}}>{roundScores.p1} — {roundScores.p2}</Text>
        </View>
      ...
      {/* Plateau */}
      <View style={{backgroundColor:'rgba(255,255,255,0.03)',borderWidth:1,borderColor:'rgba(255,255,255,0.08)',borderRadius:24,padding:16,width:'100%',marginBottom:12}}>
        {/* P1 */}
        <Text style={{fontSize:9,color:p1Color,letterSpacing:2,textAlign:'center',marginBottom:8,opacity:0.8}}>
          {t.player1}{currentPlayer===1&&phase==='selectCase'?' ● '+t.tapToPlay:''}
        </Text>
        <View style={{flexDirection:'row',gap:6,justifyContent:'center',marginBottom:4}}>
          {board.p1.map((stones,i)=>(
            <KayPit key={i} stones={stones} player={1} color={color} p1Color={p1Color} p2Color={p2Color}
              isActive={currentPlayer===1&&(phase==='selectCase'||(phase==='playing'&&isManuel))}
        ...
        <Text style={{textAlign:'center',padding:8,color:'rgba(249,242,231,0.1)',fontSize:11,letterSpacing:6}}>← ← ← ← ←</Text>
        <View style={{flexDirection:'row',gap:6,justifyContent:'center',marginBottom:4}}>
          {board.p2.map((stones,i)=>(
            <KayPit key={i} stones={stones} player={2} color={color} p1Color={p1Color} p2Color={p2Color}
              isActive={currentPlayer===2&&(phase==='selectCase'||(phase==='playing'&&isManuel))}
        ...
        <Text style={{fontSize:9,color:p2Color,letterSpacing:2,textAlign:'center',marginTop:8,opacity:0.8}}>
          {currentPlayer===2&&phase==='selectCase'?t.tapToPlay+' ● ':''}{t.player2}
        </Text>
      </View>

      {/* Badge joueur */}
      {(phase==='selectCase'||phase==='playing')&&(
        <View style={{backgroundColor:`${activeColor}18`,borderWidth:1,borderColor:`${activeColor}40`,borderRadius:50,paddingVertical:4,paddingHorizontal:14,marginBottom:10}}>
          <Text style={{fontSize:11,color:activeColor,fontWeight:'700'}}>{activeName} {phase==='selectCase'?'↓':'⟳'}</Text>
        </View>
      )}

      {/* Pierres en main (manuel) */}
      ...
      {phase==='roundOver'&&(
        <View style={{backgroundColor:'rgba(255,255,255,0.06)',borderWidth:1,borderColor:'rgba(255,255,255,0.12)',borderRadius:20,padding:22,width:'100%',alignItems:'center',marginBottom:10}}>
          ...
          <View style={{flexDirection:'row',gap:8}}>
            <TouchableOpacity style={[s.btnR,{flex:1,marginBottom:0}]} onPress={()=>startNextRound(roundWinner)}><Text style={s.btnRT}>{t.nextRound}</Text></TouchableOpacity>
            <TouchableOpacity style={[s.btnV,{flex:1,marginTop:0}]} onPress={kayRestart}><Text style={s.btnVT}>{t.restart}</Text></TouchableOpacity>
          </View>
        </View>
      )}
        ...
        <View style={{backgroundColor:'rgba(255,255,255,0.04)',borderWidth:1,borderColor:'rgba(255,255,255,0.1)',borderRadius:20,padding:16,marginTop:10,width:'100%'}}>
          ...
          <View style={{flexDirection:'row',gap:8,marginBottom:16}}>
            <TouchableOpacity onPress={()=>setIsManuel(false)} style={{flex:1,padding:10,borderRadius:12,backgroundColor:!isManuel?'rgba(245,166,35,0.2)':'rgba(255,255,255,0.04)',borderWidth:1,borderColor:!isManuel?'#F5A623':'rgba(255,255,255,0.1)',alignItems:'center'}}>
              ...
              <Text style={{color:'rgba(249,242,231,0.3)',fontSize:9,marginTop:3}}>{t.autoDesc}</Text>
            </TouchableOpacity>
            <TouchableOpacity onPress={()=>setIsManuel(true)} style={{flex:1,padding:10,borderRadius:12,backgroundColor:isManuel?'rgba(167,139,250,0.2)':'rgba(255,255,255,0.04)',borderWidth:1,borderColor:isManuel?'#A78BFA':'rgba(255,255,255,0.1)',alignItems:'center'}}>
              ...
              <Text style={{color:'rgba(249,242,231,0.3)',fontSize:9,marginTop:3}}>{t.manuelDesc}</Text>
            </TouchableOpacity>
          </View>
...
function DevineGame({ langue, onRetour }) {
  ...
  function unsub(){unsubRefs.current.forEach(f=>f());unsubRefs.current=[];}
  useEffect(()=>()=>{unsub();if(timerRef.current)clearInterval(timerRef.current);},[]);
  ...
  useEffect(()=>{
    if(timerActive&&timeLeft>0){
      ...
      setTimerActive(false);handleTimeUp();
    }
  ...
  async function createSession(){
    ...
    await fbSet(`devine/${code}`,{created:Date.now(),j2joined:false,phase:'setup'});
    setScreen('waiting');
    const u=fbListen(`devine/${code}/j2joined`,val=>{
      if(val===true){unsub();setScreen('setup');}
    });
  ...
  async function joinSession(){
    ...
    await fbSet(`devine/${code}/j2joined`,true);
    setScreen('setup');
    const u=fbListen(`devine/${code}/gameState`,val=>{
      if(!val)return;
      if(val.phase==='game'){
        setMode(val.mode);setCategory(val.category);setSecretWord(val.secretWord||'');
        setTimerDuration(val.timerDuration||60);setTimeLeft(val.timerDuration||60);
        setScreen('game');startGameListeners(code,false);unsub();
        setTimerActive(true);
      }
    });
    unsubRefs.current.push(u);
  ...
  async function startGame(){
    if(!mode)return;
    if(mode==='ia'&&!category)return;
    let word='';
    if(mode==='ia'){
      const list=DEVINE_MOTS[langue]?.[category]||[];
    ...
    setSecretWord(word);
    const state={phase:'game',mode,category:category||'',secretWord:word,timerDuration,qCount:0,winner:null};
    await fbSet(`devine/${sessionCode}/gameState`,state);
  ...
  function startGameListeners(code,amJ1){
    if(amJ1){
      const u=fbListen(`devine/${code}/currentQuestion`,val=>{
        if(val&&val.text)setPendingQ(val);
      });
      unsubRefs.current.push(u);
    }
    if(!amJ1){
      const u=fbListen(`devine/${code}/currentAnswer`,val=>{
        if(val&&val.answer){
          setHistory(h=>[...h,{q:val.q,answer:val.answer}]);
          setPendingQ(null);setQCount(c=>c+1);
        }
      });
      unsubRefs.current.push(u);
    }
    const u2=fbListen(`devine/${code}/gameState/winner`,val=>{if(val)setWinner(val);});
    const u3=fbListen(`devine/${code}/gameState/revealWord`,val=>{
      if(val){setRevealWord(val);setScreen('result');setTimerActive(false);unsub();}
    });
  ...
  async function sendQuestion(){
    ...
    await fbSet(`devine/${sessionCode}/currentQuestion`,{text:currentQ.trim(),ts:Date.now()});
    setCurrentQ('');
  }
  ...
  async function sendAnswer(answer){
    ...
    await fbSet(`devine/${sessionCode}/currentQuestion`,null);
    setPendingQ(null);
  }
  ...
  async function sendGuess(){
    ...
    if(correct){
      ...
      setLastGuessResult('wrong');setGuessInput('');setGuessMode(false);
    }
  }
  ...
  function reset(){
    unsub();if(timerRef.current)clearInterval(timerRef.current);
    setScreen('lobby');setSessionCode('');setCodeInput('');setIsJ1(true);
    setMode(null);setCategory(null);setSecretWord('');setCustomWord('');
    setTimerDuration(60);setTimeLeft(60);setTimerActive(false);
    setHistory([]);setCurrentQ('');setPendingQ(null);setQCount(0);
    setGuessMode(false);setGuessInput('');setLastGuessResult(null);
    setWinner(null);setRevealWord('');
  }

  const timerColor=timeLeft<=10?'#FF3D6B':timeLeft<=30?'#F5A623':'#10D9A0';

  return(
    <ScrollView style={s.ecran} contentContainerStyle={[s.ecranC,{paddingBottom:60}]}>
      <TouchableOpacity onPress={()=>{reset();onRetour();}} style={{marginBottom:8}}>
        <Text style={s.retour}>{t.back}</Text>
      </TouchableOpacity>
      ...
      {screen==='lobby'&&(
        <View>
          <TouchableOpacity style={[s.btnR,{backgroundColor:'#F5A623'}]} onPress={createSession}>
            <Text style={s.btnRT}>🔗 {t.createSession}</Text>
          </TouchableOpacity>
          <View style={s.sep}><View style={s.ligne}/><Text style={s.ou}>OU</Text><View style={s.ligne}/></View>
          <View style={s.joinBox}>
      ...
      {screen==='waiting'&&(
        <View style={{alignItems:'center'}}>
          <Text style={s.bigE}>⏳</Text>
          <Text style={s.desc}>{t.waitingPartner}</Text>
      ...
      {screen==='setup'&&isJ1&&(
        <View>
          ...
          {/* Mode IA */}
          <TouchableOpacity onPress={()=>setMode('ia')} style={[s.niveauCard,mode==='ia'&&{borderColor:'#F5A623',backgroundColor:'rgba(245,166,35,0.15)'},{ marginBottom:10}]}>
            <Text style={[s.niveauNom,mode==='ia'&&{color:'#F5A623'}]}>{t.modeIA}</Text>
          ...
          {/* Mode Perso */}
          <TouchableOpacity onPress={()=>setMode('perso')} style={[s.niveauCard,mode==='perso'&&{borderColor:'#10D9A0',backgroundColor:'rgba(16,217,160,0.15)'},{ marginBottom:16}]}>
            <Text style={[s.niveauNom,mode==='perso'&&{color:'#10D9A0'}]}>{t.modePerso}</Text>
          ...
          {mode==='ia'&&(
            <View style={{marginBottom:16}}>
              ...
              {cats.map(cat=>(
                <TouchableOpacity key={cat} onPress={()=>setCategory(cat)} style={[s.niveauCard,category===cat&&{borderColor:'#10D9A0',backgroundColor:'rgba(16,217,160,0.15)'},{ marginBottom:8}]}>
                  <Text style={[s.niveauNom,category===cat&&{color:'#10D9A0'}]}>{cat}</Text>
                </TouchableOpacity>
              ))}
            </View>
          )}
          ...
          {/* Mot personnalisé */}
          {mode==='perso'&&(
            <View style={[s.joinBox,{marginBottom:16}]}>
              <Text style={s.label}>{t.enterWord}</Text>
              <TextInput style={s.input} placeholder={t.wordPlaceholder} placeholderTextColor="rgba(249,242,231,0.3)" value={customWord} onChangeText={setCustomWord} autoCapitalize="none"/>
            </View>
          )}
          ...
          <View style={{flexDirection:'row',gap:8,marginBottom:20}}>
            {[30,60,90].map(sec=>(
              <TouchableOpacity key={sec} onPress={()=>setTimerDuration(sec)} style={{flex:1,padding:12,borderRadius:12,backgroundColor:timerDuration===sec?'rgba(245,166,35,0.2)':'rgba(255,255,255,0.04)',borderWidth:1,borderColor:timerDuration===sec?'#F5A623':'rgba(255,255,255,0.1)',alignItems:'center'}}>
                <Text style={{color:timerDuration===sec?'#F5A623':'rgba(249,242,231,0.5)',fontWeight:'700',fontSize:13}}>{sec===30?t.sec30:sec===60?t.sec60:t.sec90}</Text>
              </TouchableOpacity>
            ))}
          </View>

          <TouchableOpacity style={[s.btnR,{backgroundColor:'#F5A623',opacity:(!mode||(mode==='ia'&&!category)||(mode==='perso'&&!customWord.trim()))?0.4:1}]} onPress={startGame}>
            <Text style={s.btnRT}>🚀 Lancer →</Text>
          </TouchableOpacity>
        </View>
      )}
      ...
      {screen==='setup'&&!isJ1&&(
        <View style={{alignItems:'center'}}>
          <Text style={s.bigE}>⏳</Text>
          <Text style={s.desc}>{t.waitJ1}</Text>
        ...
        <View>
          ...
          <View style={{flexDirection:'row',justifyContent:'space-between',alignItems:'center',marginBottom:16,flexWrap:'wrap',gap:8}}>
            ...
            <View style={{backgroundColor:`${timerColor}18`,borderWidth:1,borderColor:`${timerColor}50`,borderRadius:50,paddingVertical:4,paddingHorizontal:14}}>
              <Text style={{color:timerColor,fontSize:14,fontWeight:'900'}}>⏱️ {timeLeft}s</Text>
            </View>
            <View style={{backgroundColor:'rgba(245,166,35,0.1)',borderWidth:1,borderColor:'rgba(245,166,35,0.3)',borderRadius:50,paddingVertical:4,paddingHorizontal:14}}>
              <Text style={{color:'#F5A623',fontSize:11}}>{qCount} {t.questions}</Text>
            </View>
          </View>

          {/* Alerte temps */}
          {timeLeft<=10&&timerActive&&(
            <View style={{backgroundColor:'rgba(255,61,107,0.15)',borderWidth:1,borderColor:'rgba(255,61,107,0.5)',borderRadius:12,padding:10,marginBottom:12,alignItems:'center'}}>
              <Text style={{color:'#FF3D6B',fontWeight:'700',fontSize:14}}>⚠️ {t.timeUp}</Text>
            </View>
          )}

          {/* Mot secret J1 */}
          {isJ1&&secretWord&&(
            <View style={{backgroundColor:'rgba(16,217,160,0.08)',borderWidth:1,borderColor:'rgba(16,217,160,0.3)',borderRadius:16,padding:16,marginBottom:16,alignItems:'center'}}>
              <Text style={{fontSize:11,color:'#10D9A0',letterSpacing:2,marginBottom:6}}>{t.theWordIs}</Text>
              <Text style={{fontSize:26,fontWeight:'900',color:'#F9F2E7',letterSpacing:2}}>{secretWord.toUpperCase()}</Text>
              <Text style={{fontSize:11,color:'rgba(249,242,231,0.3)',marginTop:6}}>{t.keepSecret}</Text>
            </View>
          )}

          {/* J1 répond */}
          {isJ1&&pendingQ&&(
            <View style={{backgroundColor:'rgba(255,61,107,0.08)',borderWidth:1,borderColor:'rgba(255,61,107,0.3)',borderRadius:20,padding:20,marginBottom:16}}>
              <Text style={{fontSize:11,color:'#FF3D6B',letterSpacing:2,marginBottom:10}}>Q#{qCount+1}</Text>
              <Text style={{fontSize:17,color:'#F9F2E7',fontWeight:'600',marginBottom:16,lineHeight:24}}>"{pendingQ.text}"</Text>
              <View style={{flexDirection:'row',gap:8}}>
                {['answerYes','answerNo','answerSometimes'].map(key=>(
                  <TouchableOpacity key={key} onPress={()=>sendAnswer(t[key])} style={{flex:1,backgroundColor:key==='answerYes'?'rgba(16,217,160,0.15)':key==='answerNo'?'rgba(255,61,107,0.15)':'rgba(245,166,35,0.15)',borderWidth:1,borderColor:key==='answerYes'?'rgba(16,217,160,0.5)':key==='answerNo'?'rgba(255,61,107,0.5)':'rgba(245,166,35,0.5)',borderRadius:12,padding:12,alignItems:'center'}}>
                    <Text style={{color:'#F9F2E7',fontSize:12,fontWeight:'700'}}>{t[key]}</Text>
                  </TouchableOpacity>
                ))}
              </View>
            </View>
          ...
          {!isJ1&&!guessMode&&(
            <View style={{marginBottom:16}}>
              {lastGuessResult==='wrong'&&(
                <View style={{backgroundColor:'rgba(255,61,107,0.1)',borderWidth:1,borderColor:'rgba(255,61,107,0.3)',borderRadius:12,padding:10,marginBottom:12,alignItems:'center'}}>
                  <Text style={{color:'#FF3D6B',fontSize:13}}>{t.wrongGuess}</Text>
                </View>
              )}
              ...
              <View style={{flexDirection:'row',gap:8}}>
                <TouchableOpacity style={[s.btnR,{flex:2,marginBottom:0,backgroundColor:'#F5A623'}]} onPress={sendQuestion}>
                  <Text style={s.btnRT}>{t.sendQuestion}</Text>
                </TouchableOpacity>
                <TouchableOpacity style={{flex:1,backgroundColor:'rgba(167,139,250,0.15)',borderWidth:1,borderColor:'rgba(167,139,250,0.4)',borderRadius:50,padding:12,alignItems:'center'}} onPress={()=>setGuessMode(true)}>
                  <Text style={{color:'#A78BFA',fontSize:12,fontWeight:'700'}}>{t.tryGuess}</Text>
                </TouchableOpacity>
              </View>
            </View>
          )}
          ...
          {!isJ1&&guessMode&&(
            <View style={{backgroundColor:'rgba(167,139,250,0.1)',borderWidth:1,borderColor:'rgba(167,139,250,0.4)',borderRadius:16,padding:16,marginBottom:16}}>
              ...
              <View style={{flexDirection:'row',gap:8}}>
                <TouchableOpacity style={[s.btnR,{flex:2,marginBottom:0,backgroundColor:'#A78BFA'}]} onPress={sendGuess}><Text style={s.btnRT}>{t.confirmGuess}</Text></TouchableOpacity>
                <TouchableOpacity style={[s.btnV,{flex:1,marginTop:0}]} onPress={()=>setGuessMode(false)}><Text style={s.btnVT}>←</Text></TouchableOpacity>
              </View>
            </View>
          )}
          ...
          {/* Abandonner */}
          {!isJ1&&(
            <TouchableOpacity onPress={giveUp} style={{alignItems:'center',marginBottom:12}}>
              <Text style={{color:'rgba(249,242,231,0.3)',fontSize:12}}>{t.giveUp}</Text>
            </TouchableOpacity>
          )}
      ...
      {screen==='result'&&(
        <View style={{alignItems:'center',backgroundColor:'rgba(255,255,255,0.04)',borderWidth:1,borderColor:'rgba(255,255,255,0.1)',borderRadius:24,padding:32}}>
          ...
          <View style={{backgroundColor:'rgba(245,166,35,0.18)',borderWidth:2,borderColor:'rgba(245,166,35,0.5)',borderRadius:16,padding:16,marginBottom:20,alignItems:'center'}}>
            <Text style={{fontSize:28,fontWeight:'900',color:'#F5A623',letterSpacing:3}}>{(revealWord||secretWord).toUpperCase()}</Text>
          </View>
          <Text style={{color:'rgba(249,242,231,0.4)',fontSize:12,marginBottom:20}}>{qCount} {t.questions}</Text>
          <TouchableOpacity style={[s.btnR,{backgroundColor:'#F5A623'}]} onPress={reset}><Text style={s.btnRT}>{t.playAgain}</Text></TouchableOpacity>
        </View>
      )}
    </ScrollView>
  );
    ...
    <SafeAreaView style={s.fond}>
      ...
      {ecran==='accueil' && (
        <Accueil langue={langue}
          onJeu={id=>{setJeuActif(id);setEcran('jeu');}}
          onChangerLangue={()=>setEcran('langue')}
        />
      )}

      {ecran==='jeu' && jeuActif==='quiz' && (
        <>
          {!session && <QuizConnexion t={t} onRetour={allerAccueil} onDemarrer={(code,j1)=>{setSession(code);setEstJ1(j1);setEcran('quiz-choix');}}/>}
        </>
      )}
...
const s = StyleSheet.create({
  ...
  jeuCard:{flexDirection:'row',alignItems:'center',gap:14,backgroundColor:'rgba(255,255,255,0.04)',borderWidth:1,borderRadius:20,padding:16,width:'100%',maxWidth:340,marginBottom:12},
